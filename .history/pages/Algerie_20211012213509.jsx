...
export default withApollo(withSearchkit(withSearchkitRouting(Search, {
  ...
  createURL: ({ qsModule, location, routeState }) => {
    ...
    const queryString = qsModule.stringify(newRouteState || "3200g", {
      addQueryPrefix: true, 
    }) 
  ...
  parseURL: ({ qsModule, location }) => {
    ...
    const matches = baseurl == "comparatifdz.com" ? location.pathname.replace('+', '1').match(/Prix-Algerie\/(\w+)/)
    : location.pathname.replace('+', '1').match(/Prix-Algerie\/(\w+)/)
