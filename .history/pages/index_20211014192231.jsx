...
const myCustomStateToRouteFn = (searchState) => {
  ...
  export default withApollo(withSearchkit(withSearchkitRouting
    ...
    parseURL: ({ qsModule, location }) => {
      ...
      const routeState = qsModule.parse("query=" + location.href.replace('-Prix-Algerie', '').replace('http://localhost:3000/', ''), { arrayLimit: 99 })
      console.log(location.href.replace('-Prix-Algerie', '').replace('http://localhost:3000/', ''))
      console.log("?" + location.href)
      console.log("?" + location.search)

      console.log("query=" + location.search.replace('-Prix-Algerie', '').replace('?', ''))
