...
// Nodejs HTTP
if (typeof window !== 'undefined' && typeof window.navigator !== 'undefined')console.log(isbot(navigator.userAgent) ? "true" : "false")
...
const myCustomStateToRouteFn = (searchState) => {
  ...
  export default withApollo(withSearchkit(withSearchkitRouting
    ...
    createURL: ({ qsModule, location, routeState }) => {
      ...
      return `/${queryString.replace('query=', '')}${price}${typeCategoryURL}`}
    },
    parseURL: ({ qsModule, location }) => {
        
      const matches = location.href.replace('+', '1').match(/Prix-Algerie\/(\w+)/)
      const routeState = qsModule.parse("query=" + location.search.split('-Prix-Algerie')[0].slice(1), { arrayLimit: 99 })
