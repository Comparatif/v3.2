...
export default withApollo(withSearchkit(withSearchkitRouting(Search, {
  ...
  createURL: ({ qsModule, location, routeState }) => {
    ...
    let CountryCategoryURL = "Algerie"
    if (routeState.filters) {
      filters = (routeState.filters).reduce((sum, filter) => {
        if (filter.identifier === "product_country") {
          sum.product_country.push(filter)
        } else {
          sum.all.push(filter)
        }
        return sum
      ...
      if (filters.product_country.length > 0) {
        CountryCategoryURL = filters.product_country.map((filter) => filter.value).join("+")
      }
    }
    ...
    return `/${CountryCategoryURL}/informatique/${typeCategoryURL}${queryString}`}
  },
