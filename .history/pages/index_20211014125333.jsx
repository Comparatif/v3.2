...
const myCustomStateToRouteFn = (searchState) => {
  ...
  export default withApollo(withSearchkit(withSearchkitRouting
    ...
    parseURL: ({ qsModule, location }) => {
      const matches = location.href.replace('+', '1').replace('query=', '').match(/localhost:3000\/(\w+)/)
      const routeState = qsModule.parse(location.search.replace('query=', '').slice(1), { arrayLimit: 99 })
