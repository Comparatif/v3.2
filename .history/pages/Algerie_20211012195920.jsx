...
export default withApollo(withSearchkit(withSearchkitRouting(Search, {
  ...
  parseURL: ({ qsModule, location }) => {
    ...
    const matches = baseurl == "localhost:3000" ? location.pathname.replace('+', '1').match(/localhost:3000\/(\w+)/)
    : location.pathname.replace('+', '1').match(/comparatifdz.com\/(\w+)/)

    const matches = baseurl == "comparatifdz.com" ? location.pathname.replace('+', '1').match(/localhost:3000\/(\w+)/)
    : location.pathname.replace('+', '1').match(/comparatifdz.com\/(\w+)/)
    console.log(matches)
    console.log(baseurl)
