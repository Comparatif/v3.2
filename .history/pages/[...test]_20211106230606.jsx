...
`
  ...
  const { previousData, data = previousData, loading } = useQuery(query, { variables })
    ...
    const routeState = {
      ...
      sort: data?.results.summary.total == 0 ? 'relevance' : 'prix_croissant',
  ...
  export default withApollo(withSearchkit(withSearchkitRouting
    ...
    createURL: ({ qsModule, location, routeState }) => {
      ...
      return `/${queryString.replace('query=', '').split('?fbclid')[0]}`
    },
    parseURL: ({ qsModule, location }) => {
      ...
      const routeState = qsModule.parse("query=" + s2, { arrayLimit: 99 })
