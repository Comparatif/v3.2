...
export default withApollo(withSearchkit(withSearchkitRouting(Search, {
  ...
  createURL: ({ qsModule, location, routeState }) => {
    ...
    return `/${queryString}/Prix-Algerie/${typeCategoryURL}`}
  },
  parseURL: ({ qsModule, location }) => {
   
    const matches = location.pathname.replace('+', '1').replace('Prix-Algerie/', '')
    .match(/localhost:3000\/(\w+)/)
    const routeState = qsModule.parse(location.search.replace('Prix-Algerie/', '').slice(1), { arrayLimit: 99 })
    console.log(qsModule.parse(location.search.slice(1), { arrayLimit: 99 }))
