...
`
  ...
  const SortedBy = data?.results.hits.sortedBy
  const Total = data?.results.summary.total
  console.log(Total)
  const [Sort, setSort] = useState("prix_croissant")
  const NoResults = <><div className="row content-justify-center text-center">
  ...
  useEffect(()=> {
    ...
    if (Total == 0 && SortedBy == "prix_croissant") {
     setSort("pertinence")
    }
    ...
    return (
      ...
      
      {
        ...
        
       {
        ...
        const test1 = !names_bdd ? product_names +  " prix Algérie - Comparaison des prix" : names_bdd + " prix Algérie - Comparaison des prix"
        const test2 = (!names_bdd ? product_names : names_bdd) + " prix Algérie - Comparaison des prix"
        return( <>
          <Head>
      {!names_bdd ? <title>{product_names} prix Algérie - Comparaison des prix</title> : <title>{names_bdd} prix Algérie - Comparaison des prix</title>} 
