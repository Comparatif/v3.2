...
const Page = () => {
  ...
  const { previousData, data = previousData, loading } = useQuery(query, { variables })
    ...
    return (
      ...
      <meta name="description" content={names + " prix Algérie | " + description1  + " | " + categories + " | " + "Meilleur prix : " + price + " DA"}/>
...
{ data?.results.summary.total == 0 ?
  ...
  {(data?.results?.hits?.sortedBy == "relevance" && data?.results.summary.total == 0 ) ?
    ...
    <HitsList data={data}  description1={description1}
    brand_name={brand_name} description2={description2} prod_description={prod_description}
    prod_specs_big_title={prod_specs_big_title} total={total} names_bdd={names_bdd} />
