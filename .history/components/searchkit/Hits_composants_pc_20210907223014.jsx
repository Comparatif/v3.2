...
import {UserConnected} from "../firebase/";
...
{data?.results.hits.items.map((hit) => (
  ...
  <a style={{cursor : "pointer"}} data-bs-toggle="modal" data-bs-target={"#" + hit.id.replace(/[0-9]/, 'Z') + "-img"}>
    ...
    <div class="row justify-content-center text-center" >
    {votesLoading ? "" :<AverageStars onClick={Canvas("SignUp-form-" + hit.id)}
    StarRating={StarRating} ListeNotesBoutique={ListeNotesBoutique}  
    cle={hit.id.replace(/[0-9]/, 'Z')}/>}    

{CanvasState == "SignUp-form-" + hit.id ? 
user ? <UserConnected cle={hit.id} boutique={hit.fields.vendeurs} 
