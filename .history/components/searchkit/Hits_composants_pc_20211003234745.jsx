...
export const HitsList = ({ data, description_data }) => {

const lesDescriptions = description_data?.description.hits.items.slice(0,1).map(
  (hit)=>
  
  [hit.fields.description1, hit.fields.brand_name, hit.fields.description2,hit.fields.prod_description,
    hit.fields.prod_specs_big_title, hit.fields.total]
  
  
  
  )

  const description1 = lesDescriptions.slice(0,1).map((hit)=>hit.fields.description1)
  const brand_name = lesDescriptions.slice(0,1).map((hit)=>hit.fields.brand_name)
  const description2 = lesDescriptions.slice(0,1).map((hit)=>hit.fields.description2)
  const prod_description = lesDescriptions.slice(0,1).map((hit)=>hit.fields.prod_description)
  const prod_specs_big_title = lesDescriptions.slice(0,1).map((hit)=>hit.fields.prod_specs_big_title)
  const total = lesDescriptions.slice(0,1).map((hit)=>hit.fields.total)
    ...
    <div class="row align-items-center">
      ...
      <div class="card card-blog card-plain">
                ...
                <a class="text-center">
                    ...
                    <p>
                    <small >
                  {description1}  <img src="/img/custom/info.svg" title="" data-bs-original-title="Beta : la description peut ne pas correspondre a la recherche dans certains cas."   data-bs-toggle="tooltip" data-bs-placement="right"/></small></p>
                  </a>
                ...
                if (screen.width < 767.98){
                    ...
                    {description_data?.description.hits.items.slice(0,1).map((hit)=>{
                ...
                return(<div class="description-custom">
                 {isOn2 === false ? <p class="small mb-0" >
                 <td  dangerouslySetInnerHTML={{__html: prod_description?.replace("<h2>", "<h2 class='title-fontsize-h2'")
                  .replace("<h3>", "<h3 class='title-fontsize-h2'")}} /></p> :
