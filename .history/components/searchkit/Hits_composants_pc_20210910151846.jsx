...
useEffect(() => {
  ...
  var tooltipListA = tooltipTriggerList.map(function (tooltipTriggerEl) {
    
      var tooltip = new bootstrap.Tooltip(tooltipTriggerEl, {trigger: "hover"})
      return tooltip.enable()
    
})
var tooltipListB = tooltipTriggerList.map(function (tooltipTriggerEl) {
    
  var tooltip = bootstrap.TooltipgetInstance(tooltipTriggerEl)
  return tooltip.disable()

})
CanvasState == "Off" ?  tooltipListA : 
(tooltipListB.forEach((e)=> e.disable()) );
...
useEffect(()=> 
  
  {
    ...
    var exampleTriggerEl = document.getElementById("tooltip-" + hit.id)
...
CanvasState={CanvasState} SetCanvas={SetCanvas} />
  ...
  <Auth cle={hit.id} CanvasState={CanvasState}  user={user}/>
