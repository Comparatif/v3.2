...
{data?.results.hits.items.map((hit) => {
    ...
    const doesOrderExist = async () => {
      ...
      }).catch((err)=> console.log(err),SetOrderContext({...OrderContext, isOrderTaken: false, isOrderModified: false, hit: hit, boutique:vendeurs, product_name: product_names}))
    
      : SetOrderContext({...OrderContext, isOrderAlreadyTakenisOrderTaken: false, isOrderModified: false, hit: hit, boutique:vendeurs, product_name: product_names})
    } 
...
/>
  <Auth cle={cle} CanvasState={CanvasState}  user={user}/>
    ...
    <img id="img-width" src="/blur/4.jpg"  class="img-fluid border-radius-lg" style={{filter: "blur(1rem)", width: "60%"}}/>
    </a>
...
{enableOrder ? 
  ...
  <a onClick={doesOrderExist} data-bs-toggle="modal" data-bs-target="#modal-form-order"><button type="button" class="btn bg-gradient-info mb-0 position-relative z-index-0 button-width" id="btn-fontsize" data-animation="true">Acheter</button></a>
