...
const StarRating = Resultat > 0 ? Number(Resultat.toFixed(1)) : 0
  ...
  <div className="modal fade" id={cle} tabIndex="-1" aria-labelledby={cle + "Label"} aria-hidden="true">
          ...
          <a href={shop_website}>
          <img className="img-fluid border-radius-lg " id="img-width" src={vendeurs_image} alt="" onError={(e) => { e.target.src = '/img/custom/broken-image.png' }}/>
          </a>
  ...
  <a style={{cursor : "pointer"}} data-bs-toggle="modal" data-bs-target={"#" + cle + "-img"}>
    ...
    <div className="col-lg-3 col-md-3 col-sm-3 col-3 mb-1 pe-1" >
      <h3 className={partenaire ? "ms-1 mb-0 title-fontsize-h1" : "text-gradient text-info ms-1 mb-0 title-fontsize-h1"} style={partenaire ? {color : "#f0db44"}: undefined} id="price-fontsize">{product_prices.replace(/\B(?=(\d{3})+(?!\d))/g, " ")}<small id="price-fontsize-ligne-currency"> DA</small></h3>
    ...
    <img id="img-width" src={vendeurs_image} alt={vendeurs} className="img-fluid border-radius-lg" onError={(e) => { e.target.src = '/img/custom/broken-image.png' }}/>
    </a>
...
/>
  <Auth cle={cle} CanvasState={CanvasState}  user={user}/>
    ...
    <img id="img-width" src="/blur/4.jpg"  className="img-fluid border-radius-lg" style={{filter: "blur(1rem)", width: "60%"}}/>
    </a>
...
<div  className="col-lg-12 col-md-12 col-sm-12 col-12 mb-1 text-end ps-lg-1 pe-lg-1 ps-md-1 pe-md-1 ps-sm-1 pe-sm-1 ps-1">
    {enableOrder ?
      <a onClick={doesOrderExist} data-bs-toggle="modal" data-bs-target="#modal-form-order"><button type="button" className="btn bg-gradient-success mb-0 position-relative z-index-0 button-width" id="btn-fontsize-product" data-animation="true">Acheter</button></a>
:

<button type="button" className="btn bg-gradient-success mb-0 position-relative z-index-0 button-width" id="btn-fontsize-product" data-animation="true"disabled={!enableOrder}>Acheter</button>
...
<div  className="col-lg-12 col-md-12 col-sm-12 col-12 mb-1 text-end ps-lg-1 pe-lg-1 ps-md-1 pe-md-1 ps-sm-1 pe-sm-1 ps-1" >
          ...
          target="_blank" rel="noreferrer"><button type="button" className="btn bg-gradient-info mb-0 position-relative z-index-0 button-width" id="btn-fontsize-product" data-animation="true">Voir l'offre</button></a>
