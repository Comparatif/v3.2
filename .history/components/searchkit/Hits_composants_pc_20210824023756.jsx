...
<div class='col-md-3 col-sm-3 col-3 mt-0 ps-0 text-center'>
  <Facets data={data.results}/>
  </div>

 
  <div class='col-md-5 col-sm-6 col-6 mt-0 text-center'>
...
<div class="col-md-3 col-sm-3 col-3 mt-0 pe-0 text-center">
...
{data?.results.hits.items.map((hit) => (
  ...
  <div class="modal fade" id={hit.id.replace(/^\[0-9]/, "")} tabindex="-1" aria-labelledby={hit.id.replace(/^\[0-9]/, "") + "Label"} aria-hidden="true">
          <div class="modal-dialog" id={hit.id.replace(/^\[0-9]/, "")} >
          ...
          <div class="modal fade" id={hit.id.replace(/^\[0-9]/, "") + "-img"} tabindex="-1" aria-labelledby={hit.id.replace(/^\[0-9]/, "") + "Label"} aria-hidden="true">
          <div class="modal-dialog" id={hit.id.replace(/^\[0-9]/, "") + "-img"} >
          <div class="modal-content">
  ...
  <div key={hit.id.replace(/^\[0-9]/, "")} class="container card shadow-lg ">
  
  <div class="row justify-content-center text-start" id="container-margin-padding">
  
  <div class="col-md-12 col-sm-12 col-12">
  
  <a style={{cursor : "pointer"}} data-bs-toggle="modal" data-bs-target={"#" + hit.id.replace(/^\[0-9]/, "") + "-img"}>
    ...
    <a data-bs-toggle="dropdown" id={hit.id.replace(/^\[0-9]/, "") + "drop"} style={{cursor: "pointer"}}>
    <img id="img-width" src={hit.fields.vendeurs_image} alt={hit.fields.vendeurs} class="img-fluid border-radius-lg" />
    </a>

    <ul class="dropdown-menu z-index-1" aria-labelledby={hit.id.replace(/^\[0-9]/, "")}>
          ...
          <a class="dropdown-item text-bold" data-bs-toggle="modal" data-bs-target={"#" + hit.id.replace(/^\[0-9]/, "")}>
          <li>
...
{data?.results.hits.items.map((hit) => (
  ...
  <div key={hit.id.replace(/^\[0-9]/, "")} class="card col-md-4 col-sm-4 col-4 mb-1" id="info-produit1">
  <div class="modal fade" id={hit.id.replace(/^\[0-9]/, "")} tabindex="-1" aria-labelledby={hit.id.replace(/^\[0-9]/, "") + "Label"} aria-hidden="true">
          <div class="modal-dialog" id={hit.id.replace(/^\[0-9]/, "")} >
          <div class="modal-content">
          <div class="modal-header justify-content-center">
          ...
          <div class="card-header p-0 mx-3 mt-3 position-relative z-index-1 justify-content-center row">
            <a style={{cursor : "pointer"}} data-bs-toggle="modal" data-bs-target={"#" + hit.id.replace(/^\[0-9]/, "") + "-img"} class="d-block blur-shadow-image">
              <img src={hit.fields.product_imagelinks}  class="img-fluid border-radius-lg"/>
            </a>
          </div>
    
          <div class="card-body p-1">
            <div class="text-bold" style={{minHeight:"50px"}}>
            {hit.fields.product_names}
            </div>
            <h1 class="text-gradient text-info mb-1" id="state1" id="price-fontsize">{hit.fields.product_prices.replace(/\B(?=(\d{3})+(?!\d))/g, " ")} DA</h1>
            <p class="mb-1" id={hit.fields.stocks === 'Rupture de stock' ? "Rupture_de_stock" : "En_stock" }><strong>{hit.fields.stocks}</strong></p>
    ...
    <a data-bs-toggle="dropdown" id={hit.id.replace(/^\[0-9]/, "") + "drop"} style={{cursor: "pointer"}}>
    <img id="img-width" src={hit.fields.vendeurs_image} alt={hit.fields.vendeurs} class="img-fluid border-radius-lg w-50" />
    </a>
    ...
    <ul class="dropdown-menu z-index-1" aria-labelledby={hit.id.replace(/^\[0-9]/, "")}>
          ...
          <img src="/img/custom/info.svg"/> Détails
            ...
            <a href={hit.fields.product_links} class="text-success icon-move-right pull-right">Voir produit
