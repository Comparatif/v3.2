...
export const Listes = ({ data, boutique_infos, votesLoading, user, datetime, CanvasState, SetCanvas }) => {
...
useEffect(()=> 
  ...
  <div class="modal fade" id={cle} tabindex="-1" aria-labelledby={cle + "Label"} aria-hidden="true">
          ...
          <div class="modal fade" id={cle + "-img"} tabindex="-1" aria-labelledby={hit.id.replace(/[0-9]/, 'Z') + "Label"} aria-hidden="true">
  ...
  <a style={{cursor : "pointer"}} data-bs-toggle="modal" data-bs-target={"#" + hit.id.replace(/[0-9]/, 'Z') + "-img"}>
    ...
    cle={hit.id.replace(/[0-9]/, 'Z')} CanvasState={CanvasState}/>

  
    </div>

<UserConnected cle={hit.id.replace(/[0-9]/, 'Z')} boutique={hit.fields.vendeurs} datetime={datetime} 
...
Vendeur_image={hit.fields.vendeurs_image} votesLoading={votesLoading}
/>

  <Auth cle={hit.id.replace(/[0-9]/, 'Z')} CanvasState={CanvasState}  user={user}/>
