...
export const Grilles = ({ data, boutique_infos, votesLoading, user, datetime, CanvasState, SetCanvas, isOn, toggleIsOn }) => {
  ...
  const imgWidthBlur = {maxHeight: '40px', filter: "blur(1rem)"}
...
const StarRating = Resultat > 0 ? Number(Resultat.toFixed(1)) : 0
  ...
  <div class="modal fade" id={cle} tabindex="-1" aria-labelledby={cle + "Label"} aria-hidden="true">
          ...
          <strong>Pays :</strong>  {product_country} <Image src={product_country == "Algérie" ? "/img/custom/2634506_algeria_ensign_flag_nation_icon.png" : "" } style={{width: "8%", height: "8%", borderRadius: "1px", boxShadow: "1px 1px 2px black"}}/><br/>
          ...
          <Image class="w-75" src={product_imagelinks} />
          </div>
          ...
          <div class="card-header h-50 p-0 m-2 position-relative z-index-0 justify-content-center row">
            <a style={{cursor : "pointer"}} data-bs-toggle="modal" data-bs-target={"#" + cle + "-img"} class="d-block blur-shadow-image">
              <Image src={product_imagelinks}  class="img-fluid shadow border-radius-lg w-100" style={{objectFit: "cover", aspectRatio: "1 / 1"}}/>
            </a>
          </div>
    ...
    <Image id="img-width" src={vendeurs_image} alt={vendeurs} class="img-fluid border-radius-lg " style={{maxheight: "20px"}}/>
    </a>
    ...
    <ul class="dropdown-menu z-index-1" aria-labelledby={cle}>
          ...
          <li >
          <Image src="/img/custom/phone-call.svg"/>  Appeler
          </li>
          </a>

          <a class="dropdown-item text-bold" href={localisation} target="_blank">
          <li>
          <Image src="/img/custom/map.svg"/> Localisation
          </li>
          </a>

          <a class="dropdown-item text-bold" href={shop_website} target="_blank">
          <li>
          <Image src="/img/custom/external-link.svg"/> Voir site
          </li>
          </a>
    ...
    <Image  style={{objectFit: "scale-down", aspectRatio: "1 / 1", maxWidth:"50%"}} alt= "CCP" src="/img/custom/ccp.png"/> : ""}
    {paiement?.toString().includes("Baridi") ? 
    <Image  style={{objectFit: "scale-down", aspectRatio: "1 / 1", maxWidth:"50%"}} alt="Baridimob" src="/img/custom/baridimob.png"/> : ""}
    {paiement?.toString().includes("paypal") ? 
    <Image  style={{objectFit: "scale-down", aspectRatio: "1 / 1", maxWidth:"50%"}} alt="Paypal" src="/img/custom/1156727_finance_payment_paypal_icon.png"/> : ""}
    {paiement?.toString().includes("visa") ? 
    <Image  style={{objectFit: "scale-down", aspectRatio: "1 / 1", maxWidth:"50%"}} alt="visa" src="/img/custom/206684_visa_method_card_payment_icon.png"/> : ""}
    {paiement?.toString().includes("CIB") ? 
    <Image  style={{objectFit: "scale-down", aspectRatio: "1 / 1", maxWidth:"50%"}} alt="CIB" src="/img/custom/CIB.png"/> : ""}
    {paiement?.toString().includes("DAHABIA") ? 
    <Image  style={{objectFit: "scale-down", aspectRatio: "1 / 1", maxWidth:"50%"}} alt="dahabia" src="/img/custom/edahabia_gold-300x190.png"/> : ""}
    </div>
            </div>
            <div class="col-4 p-0" >
