...
{data?.results.hits.items.map((hit) => (
  ...
  <div key={hit.id.replace(/[0-9]/, 'Z')} class="card col-lg-4 col-md-6 col-sm-6 col-6 mb-1" id="info-produit1">
  <div class="modal fade" id={hit.id.replace(/[0-9]/, 'Z')} tabindex="-1" aria-labelledby={hit.id.replace(/[0-9]/, 'Z') + "Label"} aria-hidden="true">
          <div class="modal-dialog" id={hit.id.replace(/[0-9]/, 'Z')} >
          <div class="modal-content">
          <div class="modal-header justify-content-center">
          ...
          <div class="card-header h-50 p-0 mx-3 mt-3 position-relative z-index-1 justify-content-center row">
            <a style={{cursor : "pointer"}} data-bs-toggle="modal" data-bs-target={"#" + hit.id.replace(/[0-9]/, 'Z') + "-img"} class="d-block blur-shadow-image justify-content-center">
              <img src={hit.fields.product_imagelinks}  class="img-fluid border-radius-lg" style={{objectFit: "cover", aspectRatio: "1 / 1"}}/>
            </a>
          </div>
