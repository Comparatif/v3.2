  ...
  <div className="px-sm-0">
    <div className="row align-items-center">
      ...
      <div className="card card-blog card-plain">
                ...
                <a className="text-center" href={isBrowser ? window.location.href : ''}>
                
                    <h1 className="title-fontsize-h1 " style={{borderBottom: '1px solid #000',
                  ...
                  <a className="row justify-content-center blur-shadow-image">
                    <img src={product_imagelinks} alt={product_names} className="img-fluid shadow border-radius-lg w-50" style={{objectFit: "cover", aspectRatio: "1 / 1"}}/>
                  </a>
                <div className="colored-shadow" id="card-url"></div></div>
                <div className="card-body px-1 pt-3 pb-0">
                  {CheckIfSorted == 'prix_croissant' ? <p className="text-gradient text-primary mb-2 text-sm row justify-content-center">Prix le moins cher : {product_prices} DA</p> : ''}
                  <button className="btn bg-gradient-info btn-lg w-50 py-2 px-1 " onClick={toggleIsOn2} >{isOn2 ? 'Description' : 'Spécifications'}</button>
                  <button onClick={() => {
                    if (typeof window !==  'undefined') {
                if (screen.width < 767.98){
                        document.getElementsByTagName("BUTTON")[2].scrollIntoView();
                        }}
                    }} type="button" className="btn bg-gradient-info btn-lg w-50 py-2 px-1">Comparer {data?.results.summary.total} offres</button>
