...
export const NavBar = ({data}) => {
  const lesFacets = data?.facets.map((facet) =>
  return facet.identifier === "type"
  )

  
  const api = useSearchkit();
  const [Value, SetValue] = useState('Composants_pc')
  const composants = () => {SetValue('Composants_pc'); SearchkitIndex('Composants_pc'); api.toggleFilter({identifier: facet.identifier === "type", value: "Composants_pc"}); api.search();}
  const laptop = () => {SetValue('Laptop'); SearchkitIndex('Laptop'); api.toggleFilter({identifier: facet.identifier === "type", value: "Laptop"}); api.search();}
