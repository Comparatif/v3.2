...
//filters((id, type, lab, display, entries) => id === type)
  {data?.results.facets.map((facet) => {
    ...
    facet.entries.filter((id, type, label, display, entries) => 
    id.facet === "type" && id.facet === "type").map((entry) => {

return (console.log(entry.label))
  ...
  const composants = () => {SetValue('Composants_pc'); SearchkitIndex('Composants_pc'); api.toggleFilter({filter: {identifier: "type", value: "Composants_pc"}}); api.search();}
  const laptop = () => {SetValue('Laptop'); SearchkitIndex('Laptop'); api.toggleFilter({filter: {identifier: "type", value: "Laptop"}}); api.search();}
