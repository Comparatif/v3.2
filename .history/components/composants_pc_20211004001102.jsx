...
const description_query  = gql`
  query descriptionResultSet($query: String, $filters: [SKFiltersSet], $page: SKPageInput, $sortBy: String) {
    description(query: "5600X", filters: $filters) {
      summary {
        total
        appliedFilters {
          id
...
const Page = () => {
  ...
  const { description_previousData, description_data = description_previousData, description_loading } 
  = useQuery(description_query, { variables })
...
{ data?.results.summary.total == 0 ?
  ...
  {(data?.results?.hits?.sortedBy === "relevance" && data?.results.summary.total == 0 ) ?
    ...
    <HitsList data={data} description_data={description_data} />
