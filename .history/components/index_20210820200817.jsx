...
import { UserContext, Tri, cartItemsVar } from './UserContext.js'
import {
  FacetsList,
  SearchBar,
  Pagination,
...
const query2 = gql`
  query resultSet($query: String, $filters: [SKFiltersSet], $page: SKPageInput, $sortBy: String) {
    results(query: $query, filters: $filters, queryOptions: { fields: ["or"]}) {
      summary {
        total
        appliedFilters {
          id
...
const Page = () => {
  ...
  const [valeur, setQuery] = useSearchkitQueryValue();
  ...
  const reglage1 = () => {cartItemsVar('and'); api.setQuery(valeur); api.search()}
  const reglage2 = () => {cartItemsVar('or'); api.setQuery(valeur); api.search()}
  return (
    <>
