...
data?.results.hits.items.map((hit, col) => {
  ...
  <ul className="dropdown-menu z-index-3" aria-labelledby={cle}>
    ...
    <img src="/img/custom/info.svg"/> Détails
      ...
      <i className={isSelected.row === id && isSelected.col === col ? "fas fa-credit-card mx-1 outline_icons" : "fas fa-credit-card mx-1 outline_icons"} style={{color: paiement?.toString() === "" ? "" : "green" }}
      data-bs-toggle="tooltip" data-bs-placement="top" title={paiement} data-container="body" data-animation="true"></i>
      <i className="fas fa-boxes mx-1 outline_icons" style={{color: stocks === 'Rupture de stock' ? "red" : stocks?.includes("Stock") ? "green" : stocks?.includes("command") ? "orange" : "" }} 
