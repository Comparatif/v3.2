...
/* La Barre de recherche */
  ...
  useEffect(()=> {
    const product_name = data?.results?.hits?.items.slice(0,1).map((hit)=> hit.fields.product_names)[0]
    const shops = data?.results.hits.items.map((hit) => hit.fields.vendeurs)
    const prices = data?.results.hits.items.map((hit) => hit.fields.product_prices)
    
    ExcelTable.push([data?.results?.hits?.items.slice(0,1).map((hit)=> hit.fields.product_names)[0], 
    ...data?.results.hits.items.map((hit) => hit.fields.vendeurs)
  
  ]);

  ExcelTable.push(["Prix", 
    ...data?.results.hits.items.map((hit) => hit.fields.product_prices)
  
  ])

}, [])

console.log(data?.results?.hits?.items.slice(0,1).map((hit)=> hit.fields.product_names)[0])
console.log(data?.results.hits.items.map((hit) => hit.fields.vendeurs))
