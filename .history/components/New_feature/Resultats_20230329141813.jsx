...
export const Resultats =  ({queryssetData, data, isSelected, setSelect, id}) =>
...
data?.results.hits.items.map((hit, col) => {
    ...
    return(
        ...
        <td 
          ...
          key={cle} 
          className={isSelected.row === id && isSelected.col === col ? "results_ligne border-radius-xl shadow m-1 bg-gradient-dark"  :"results_ligne border-radius-xl shadow m-1" }
      ...
      <a href={shop_website}>
      <img  className="img-fluid border-radius-lg img_white_background" id="img-width" src={vendeurs_image} alt={vendeurs}/>
      </a>
  ...
  <img id="img-width" src={vendeurs_image} alt={vendeurs} className="img-fluid border-radius-2xl img_white_background " style={{maxheight: "20px", objectFit: "scale-down", aspectRatio: "20/9"}}/>
  </a>
  ...
  <ul className="dropdown-menu z-index-3" aria-labelledby={cle}>
    ...
    <img src="/img/custom/info.svg"/> Détails
      ...
      <div style={{cursor: "pointer"}} onClick={()=> {setSelect({row: id, col: col}); setData({data: data, query: querys})}} className="row justify-content-center text-center">
