...
export const Ligne = ({data}) => {
          ...
          {data?.results?.hits?.items.slice(0,1).map((hit)=>
            {
              ...
              return(
                ...
                <a style={{cursor : "pointer"}} onClick={()=> toggleIsOn()} >
                  <h6 class="mb-0 text-s">{product_names}</h6>
                  <p class="text-s text-secondary mb-0">{!names_bdd ? "" : description1}</p>
                  </a>
                  </div>
                  </>}
                  </div></td>
  ...
  <img id="img-width" src={vendeurs_image} alt={vendeurs} class="img-fluid border-radius-lg " style={{objectFit: "cover", aspectRatio: "1/1"}}style={{maxheight: "20px"}}/>
  </a>
