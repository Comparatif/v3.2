...
data?.results.hits.items.map((hit) => {
    ...
    return(
        ...
        <td 
          ...
          key={cle} 
          className="results_ligne border-radius-xl shadow m-1 " 
          >
  ...
  <ul className="dropdown-menu z-index-3" aria-labelledby={cle}>
    ...
    <img src="/img/custom/info.svg"/> Détails
      ...
      <h3 className="text-gradient text-info mb-1 col-8 p-0 z-index-1" id="price-fontsize-ligne" ><small>{product_prices.replace(/\B(?=(\d{3})+(?!\d))/g, " ")}</small><small id="price-fontsize-ligne-currency"> DA</small></h3>
      <div>
      <i className="fas fa-boxes" style={{color: stocks === 'Rupture de stock' ? "red" : stocks?.includes("Stock") ? "green" : stocks?.includes("command") ? "orange" : "" }} 
      data-bs-toggle="tooltip" data-bs-placement="top" title={stocks} data-container="body" data-animation="true"></i>
      <i className="fas fa-truck" style={{color: stocks === 'Rupture de stock' ? "red" : stocks?.includes("Stock") ? "green" : stocks?.includes("command") ? "orange" : "" }}
      data-bs-toggle="tooltip" data-bs-placement="top" title={livraison} data-container="body" data-animation="true"></i>
