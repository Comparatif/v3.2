...
var date = (currentdate.getDate()) + (currentdate.getMonth()+1)  +  (currentdate.getFullYear())
  ...
  const doesOrderExist = async (e:any) => {
    ...
    }).catch((err)=> SetOrderContext({...OrderContext, isOrderTaken: false, isOrderAlreadyTaken: false, isOrderModified: false, hit: hit, boutique:vendeurs, product_name: product_names, error: err}))
  } 
...
try{
  ...
  const OrderConfirmation = isLastnameCompliant && isFirstnameCompliant && isPhoneCompliant && isCityCompliant && isCommuneCompliant
    ...
    const tryPrisma = async (e:any) => {
      ...
      : 
      (doesOrderExist(), OrderContext?.isOrderAlreadyTaken== true ? "":
      (create(DatabaseSeed),
      SetOrderContext({...OrderContext, isOrderTaken: true, isOrderModified: false}))
...
<div className="card card-plain">
  ...
  {OrderContext?.isOrderAlreadyTaken ? "Commande déjà prise en compte !" : <p className="mb-0 text-small text-center">
