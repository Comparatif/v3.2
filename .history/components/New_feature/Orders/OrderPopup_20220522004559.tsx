...
import { prisma } from '../../../prismaprisma/db'
...
var date = (currentdate.getDate()) + (currentdate.getMonth()+1)  +  (currentdate.getFullYear())
  ...
  const isCommuneCompliant = data.commune !== ""
  ...
  e.preventDefault()
    ...
    try {Order.get()
      ...
      if (doc.exists) {
        Order.update({
          ...
          Infos_boutique: hit?.fields,userID: user.uid, email: data.email, phoneNumber: data.phoneNumber, lastName: data.lastName, firstName: data.firstName, countryName: data.countryName, city: data.city, commune: data.commune, datetime, sold: false
      });
      SetOrderContext({...OrderContext, isOrderModified: true, isOrderTaken: true})

    } else {
      Order.set({Infos_boutique: hit?.fields, userID: user.uid, email: data.email, phoneNumber: data.phoneNumber, lastName: data.lastName, firstName: data.firstName, countryName: data.countryName, city: data.city, commune: data.commune, datetime, sold:false});
    ...
    const tryPrisma = async (e: any) => {
      ...
      await prisma.user.create({
        data: {
            email: data.email, phoneNumber: data.phoneNumber, lastName: data.lastName, firstName: data.firstName, countryName: data.countryName, city: data.city, user_commune: data.commune,
...
{!OrderContext?.isOrderTaken ? 
 ...
 Effectuer une commande
  ...
  <form role="form text-left" onSubmit={tryPrisma}>
  <div className="row">
  <div className="col-4">
  <label>Prénom</label>
...
<select onChange={(e:any)=> setData({
  ...
  {Commune[Wilaya.indexOf(data.city)]?.map((valeur)=> <option value={valeur} selected={valeur == data.commune ? true : false}>{valeur}</option>)}
</select>
</div>
</div>
