...
try{
  ...
  const OrderConfirmation = isLastnameCompliant && isFirstnameCompliant && isPhoneCompliant && isCityCompliant && isCommuneCompliant
    ...
    const tryPrisma = async (e:any) => {
      ...
      : doesOrderExist ? SetOrderContext({...OrderContext, isOrderAlreadyTaken: true, isOrderModified: false}) :create(DatabaseSeed),
      SetOrderContext({...OrderContext, isOrderTaken: true, isOrderModified: false}))
...
console.log(OrderContext?.isOrderTaken)
...
!OrderContext?.isOrderTaken ? 
 ...
 Effectuer une commande
  ...
  <form role="form text-left" onSubmit={()=>user ? tryPrisma : (doesOrderExist ? SetOrderContext({...OrderContext, isOrderTaken: true, isOrderModified: false}): tryPrisma)} method="post">
  <div className="row">
  <div className="col-4">
  <label>Prénom</label>
