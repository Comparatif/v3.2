...
var date = (currentdate.getDate()) + (currentdate.getMonth()+1)  +  (currentdate.getFullYear())
  ...
  const doesOrderExist = async (e:any) => {
    ...
    .then((data) => {
      ...
      :
      SetOrderContext({...OrderContext, isOrderTaken: false, isOrderModified: false, hit: hit, boutique:vendeurs, product_name: product_names}) 
    }).catch((err)=> SetOrderContext({...OrderContext, isOrderTaken: false, isOrderModified: false, hit: hit, boutique:vendeurs, product_name: product_names, error: err}))
  } 
...
!OrderContext?.isOrderTaken || !OrderContext?.isOrderAlreadyTaken ? 
 ...
 Effectuer une commande
  ...
  <form role="form text-left" onSubmit={user ? tryPrisma: doesOrderExist } method="post">
  <div className="row">
  <div className="col-4">
  <label>Prénom</label>
...
<select onChange={(e:any)=> setData({
  ...
  <button  type={OrderConfirmation ? "submit" : "button"} onClick={()=> OrderConfirmation ? (setOrderTrigger(false);user ? tryPrisma: doesOrderExist)  : setOrderTrigger(true)} className="btn btn-round bg-gradient-info btn-lg w-100 mt-4 mb-0">
  Commander
  </button>
  </div>
  </form>
