...
:
 ...
 Effectuer une commande
  ...
  Nous vous contacterons dans les plus brefs délais afin de vous mettre en relation avec le marchand. <img src="/img/custom/info.svg" title="" data-bs-original-title="Nous ne vendons en aucun cas le produit selectionné. Nous nous chargeons uniquement de vous mettre en relation avec le marchand."  data-bs-trigger="hover" data-bs-toggle="tooltip" data-bs-placement="right"/>
  </p>
  </div>
  <div className="card-body">
...
<div className="col-4">
  ...
  {Wilaya.map((wilaya)=> <option key={wilaya} value={wilaya} selected={wilaya == data.city ? true : false} >{wilaya}</option>)}
...
<select onChange={(e:any)=> setData({
  ...
  {Commune[Wilaya.indexOf(data.city)]?.map((valeur)=> <option key={valeur} value={valeur} selected={valeur == data.user_commune ? true : false}>{valeur}</option>)}
</select>
</div>
</div>
