    ...
    try {Order.get()
      ...
      if (doc.exists) {
        Order.update({
          ...
          Infos_boutique: hit?.fields,userID: user.uid, email: data.email, phoneNumber: data.phoneNumber, lastName: data.lastName, firstName: data.firstName, countryName: data.countryName, city: data.city, user_commune: data.user_commune, datetime, sold: false
      });
      SetOrderContext({...OrderContext, isOrderModified: true, isOrderTaken: true})

    } else {
      Order.set({Infos_boutique: hit?.fields, userID: user.uid, email: data.email, phoneNumber: data.phoneNumber, lastName: data.lastName, firstName: data.firstName, countryName: data.countryName, city: data.city, user_commune: data.user_commune, datetime, sold:false});
...
<div className="col-4">
  ...
  <select onChange={(e:any)=> setData({
    ...
    className={isCityCompliant ? "form-control is-valid" : isOrderTrigger && !isCityCompliant ? "form-control is-invalid" : "form-control"} name="choices-button-city"  placeholder="Wilaya">
...
<select onChange={(e:any)=> setData({
    ...
    className={isCommuneCompliant ? "form-control is-valid" : isOrderTrigger && !isCommuneCompliant ? "form-control is-invalid" : "form-control"} name="choices-button-commune" i placeholder="Commune" >
