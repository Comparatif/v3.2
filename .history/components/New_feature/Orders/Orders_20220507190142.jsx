...
const OrdersNumber = prix.length
const Sold = Orders?.docs?.filter((doc)=> doc.data().sold === true).length
const Pending = Orders?.docs?.filter((doc)=> doc.data().sold === false).length

return ({CA: CA, OrdersNumber: OrdersNumber, Sold: Sold, Pending:})
}
