...
try{
  ...
  const OrderConfirmation = isLastnameCompliant && isFirstnameCompliant && isPhoneCompliant && isCityCompliant && isCommuneCompliant
    ...
    const tryPrisma = async (e:any) => {
      ...
      create(DatabaseSeed)
      SetOrderContext({...OrderContext, isOrderTaken: true, isOrderModified: false})):setOrderTrigger(false)
...
<select onChange={(e:any)=> setData({
  ...
  <button  type="button" onClick={()=> OrderConfirmation ? (setOrderTrigger(false),user ? tryPrisma: doesOrderExist)  : setOrderTrigger(true)} className="btn btn-round bg-gradient-info btn-lg w-100 mt-4 mb-0">
  Commander
  </button>
  </div>
  </form>
