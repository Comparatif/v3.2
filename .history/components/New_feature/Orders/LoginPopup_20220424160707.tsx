...

{
  ...
  const handlesendPasswordReset = async (e: any) => {
    e.preventDefault()
    try {
      await sendPasswordReset(data.emailRecovery)
    } catch (err) {
      console.log(err)
    }
  }
...
//without user
 ...
 {PasswordLost ? "Récuperation du mot de passe" : "Connectez-vous"} 
  ...
  {PasswordLost ? "Veuillez entrer votre adresse e-mail puis entrez le code de vérification ainsi que le nouveau mot de passe." 
