...
try{
  ...
  e.preventDefault()
    ...
    const DatabaseSeed= Object.keys(Object.assign(data, hit?.fields)).map(({ email, ...e }) => ({ ...e, shopEmail: email }))
    console.log(DatabaseSeed.map(({ email, ...e }) => ({ ...e, shopEmail: email })))
    const tryPrisma = async () => {


      create(DatabaseSeed.map(({ email, ...e }) => ({ ...e, shopEmail: email })))
