...
var date = (currentdate.getDate()) + (currentdate.getMonth()+1)  +  (currentdate.getFullYear())
  ...
  const id = user ? idWithAccount : idWithoutAccount

  const doesOrderExist = async () => {
    
    fetch(`/api/verify-existance?searchString=${id}`, {
      headers : { 
...
try{
  ...
  const OrderConfirmation = isLastnameCompliant && isFirstnameCompliant && isPhoneCompliant && isCityCompliant && isCommuneCompliant
    ...
    const tryPrisma = async (e:any) => {
      ...
      const  DatabaseSeed = Object.assign(Object.assign(data, hit?.fields), {orderId : id, shopEmail: shopEmail, shop_commune: shop_commune})
