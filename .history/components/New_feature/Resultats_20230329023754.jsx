...
data?.results.hits.items.map((hit, col) => {
  ...
  <ul className="dropdown-menu z-index-3" aria-labelledby={cle}>
    ...
    <img src="/img/custom/info.svg"/> Détails
      ...
      <h3 className={isSelected.row === id && isSelected.col === col ? "text-info mb-1 col-8 p-0 z-index-1 text-light" : "text-gradient text-info mb-1 col-8 p-0 z-index-1"} id="price-fontsize-ligne" ><small>{product_prices.replace(/\B(?=(\d{3})+(?!\d))/g, " ")}</small><small id="price-fontsize-ligne-currency"> DA</small></h3>
