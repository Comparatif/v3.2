...
const [desc, setDesc] = useState({a: null, b: null, c:null, d:null, e:null, f:null, g:null})

const CheckIfSorted = data?.results?.hits.sortedBy

useEffect(()=> {
data?.description?.hits?.items?.slice(0,1).map(
  (hit)=>

  setDesc({a: hit?.fields.description1, b: hit?.fields.brand_name, c: hit?.fields.description2, d: hit?.fields.prod_description,
    e: hit?.fields.prod_specs_big_title, f: hit?.fields.total, g: hit?.fields.product_names})
  )
},[data])


  const description1 = desc?.a
  const prod_description = desc?.d
  const total = desc?.f
  const names_bdd = desc?.g
  console.log()
...
{data?.data?.results?.hits?.items.slice(0,1).map((hit)=>
  {
