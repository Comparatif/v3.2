    ...
    
{
  ...
  const lesDescriptions =  data?.description?.hits?.items?.slice(0,1).map(
    ...
    if (id ===1) {console.log({id: id, ShowSearchBar: ShowSearchBar, isOn: !isOn, getQuery: api.getQuery() == "", CheckIfZeroResults: CheckIfZeroResults})}
    if (id ===2) {console.log({id: id, ShowSearchBar: ShowSearchBar, isOn: !isOn, getQuery: api.getQuery() == "", CheckIfZeroResults: CheckIfZeroResults})}
  

   return(
    <th className="searchbar_ligne" >
    
<div className="d-flex px-2 py-1">
{ShowSearchBar ?
<>
<div className="d-flex flex-column col-2 justify-content-center">
  <a onClick={()=>toggleIsOn()} style={{cursor: 'pointer'}} >
  <small className="row justify-content-center col-11 m-0">
...
<div className="d-flex flex-column col-8 justify-content-center me-1">
<a className="pt-2" style={{cursor : "pointer"}} onClick={()=> toggleIsOn()} >
  ...
  <p className="text-xs text-secondary mb-0 mt-2">Tri: {CheckIfSorted == "prix_croissant" ? "Par Prix" : "Pertinence"}</p>
