...
/* La Barre de recherche */
  ...
  // Add names to excel context
    ...
    useEffect(()=> {
      ...
      const product_names = data?.results?.hits?.items.slice(0,1).map((hit)=> hit.fields.product_names)
      ...
      const test = product_names?.push.apply(product_names,shops)
      const test2 = [...[product_names], ...[shops]]
      const test3 = product_names?.concat([shops])
      console.log(test)
      console.log(test2)
      console.log(test3)

      const list = data?.results?.hits?.items.slice(0,1).map((hit)=> hit.fields.product_names).concat(data?.results.hits.items.map((hit) => hit.fields.vendeurs))
    },[data])
