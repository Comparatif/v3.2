...
/* La Barre de recherche */
    ...
    const SearchBar = data?.results?.hits?.items.slice(0,1).map((hit)=>
  {
...
<ul class="dropdown-menu z-index-1" aria-labelledby={cle}>
  ...
  <img src="/img/custom/info.svg"/> Détails
    ...
    const NoResults = <td class="results_ligne">{SearchBar}<h1>Aucun résultats, réessayez :(</h1></td>
    const CheckIfZeroResults = data?.results.summary.total == 0
    const RelevanceAndZero = (data?.results?.hits?.sortedBy == "relevance" && data?.results.summary.total == 0 )
    const RelevanceSearch = api.setSortBy('relevance') && SearchBar && resultats
    ...
    return(
          ...
          <tr>{
            ...
            loading ? ChargementLogo : CheckIfZeroResults ? 
              
              RelevanceAndZero ? NoResults : RelevanceSearch
