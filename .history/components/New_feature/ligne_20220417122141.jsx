...
export const Ligne = ({data, loading}) => {
...
/* La Barre de recherche */
    const ShowSearchBar = !isOn || api.getQuery() == "" || CheckIfZeroResults ==0
    const LeftField = 
...
<ul class="dropdown-menu z-index-1" aria-labelledby={cle}>
  ...
  <img src="/img/custom/info.svg"/> Détails
    ...
    const CheckRelevance = data?.results?.hits?.sortedBy
    ...
    const Search = <>{LeftField}  {resultats}</>
    ...
    return(
      ...
      <tr>
      {loading ? LoadingLogo : data?.results.summary.total == 0 ? 
        
        data?.results?.hits?.sortedBy == "relevance" && data?.results.summary.total == 0 ?  NoResults  : <> {api.setSortBy('relevance')} {Search} </>: Search }
        </tr>
