...
export const Resultats =  ({data1, setData, data, isSelected, setSelect, id}) =>
...
data?.results.hits.items.map((hit, col) => {
    ...
    return(
      ...
      <a href={shop_website}>
      <img  className="img-fluid border-radius-lg img_white_background" id="img-width" 
      src={vendeurs_image} onError={(e) => { e.target.src = '/img/custom/broken-image.png' }} alt={vendeurs}
      data-bs-toggle="tooltip" data-bs-placement="top" title={vendeurs} data-container="body" data-animation="true"/>
      </a>
  ...
  <img id="img-width" src={vendeurs_image} onError={(e) => { e.target.src = '/img/custom/broken-image.png' }} alt={vendeurs} className="img-fluid border-radius-2xl img_white_background " style={{maxheight: "20px", objectFit: "scale-down", aspectRatio: "20/9"}}
  data-bs-toggle="tooltip" data-bs-placement="top" title={vendeurs} data-container="body" data-animation="true"/>
  </a>
  ...
  <ul className="dropdown-menu z-index-3" aria-labelledby={cle}>
    ...
    <img src="/img/custom/info.svg"/> Détails
      ...
      <div style={{cursor: "pointer"}} onClick={()=> {setSelect({row: id, col: col}); setData({...data1.data, data: data})}} className="row justify-content-center text-center">
