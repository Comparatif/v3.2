...
const Ligne = ({id, SearchState, isOn4, excelData, setExcelData}) => {
  const query  = gql`
...
/* La Barre de recherche */
  ...
  useEffect(()=> {
    ...
    const list1 = data?.results?.hits?.items.slice(0,1).map((hit)=> hit.fields.product_names).concat(data?.results.hits.items.map((hit) => hit.fields.vendeurs)).unshift(id-1)
    const list2 = ["Prix"].concat(data?.results.hits.items.map((hit) => hit.fields.product_prices)).unshift(id-1)

    const exisitingAmenIdx = updatedAmen.findIndex((a) => a.name === type);
    if (exisitingAmenIdx >= 0) {
      setExcelData(excelData => [...excelData, list1, list2])
