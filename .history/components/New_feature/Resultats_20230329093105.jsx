export const Resultats =  ({data, isSelected, setSelect, id}) =>
...
data?.results.hits.items.map((hit, col) => {
    ...
    return(
        ...
        <td 
          ...
          key={cle} 
          className={isSelected.row === id && isSelected.col === col ? "results_ligne border-radius-xl shadow m-1 bg-gradient-info"  :"results_ligne border-radius-xl shadow m-1" }
  ...
  <img id="img-width" src={vendeurs_image} alt={vendeurs} className="img-fluid border-radius-2xl " style={{maxheight: "20px", objectFit: "scale-down", aspectRatio: "20/9"}}/>
  </a>
  ...
  <ul className="dropdown-menu z-index-3" aria-labelledby={cle}>
    ...
    <img src="/img/custom/info.svg"/> Détails
      ...
      <div style={{cursor: "pointer"}} onClick={()=> setSelect({row: id, col: col})} className="row justify-content-center text-center">
      ...
      <i className={isSelected.row === id && isSelected.col === col ? "fas fa-credit-card mx-1 outline_icons" : "fas fa-credit-card mx-1"} style={{color: paiement?.toString() === "" ? "" : "green" }}
      data-bs-toggle="tooltip" data-bs-placement="top" title={paiement} data-container="body" data-animation="true"></i>
      <i className={isSelected.row === id && isSelected.col === col ? "fas fa-boxes mx-1 outline_icons" : "fas fa-boxes mx-1"} style={{color: stocks === 'Rupture de stock' ? "red" : stocks?.includes("Stock") ? "green" : stocks?.includes("command") ? "orange" : "" }} 
      data-bs-toggle="tooltip" data-bs-placement="top" title={stocks} data-container="body" data-animation="true"></i>
      <i className={{isSelected.row === id && isSelected.col === col ? "fas fa-credit-card mx-1 outline_icons" : "fas fa-credit-card mx-1"}} style={{color: livraison?.toString() === '' ? "" : "green" }}
