...
`
  ...
  //const { previousData2, data2 = previousData2, loading2 } = useQuery(query2, { variables })
    ...
    const CheckIfZeroResults = data?.results?.summary.total
...
/* La Barre de recherche */
    ...
    return(
            ...
            <tr>
           <button onClick={()=> {api.setSortBy('relevance'); api.search()}}></button>
           <button onClick={()=> {api.setSortBy('prix_croissant'); api.search()}}></button>
            {<LeftField data={data} isOn ={isOn} toggleIsOn={toggleIsOn} setQuery={setQuery} liste={liste} IndexChange={IndexChange} />}
            {<div><p>{CheckIfZeroResults}</p></div>}
            {loading ? LoadingLogo : CheckIfZeroResults == 0 ? <>{api.setSortBy('relevance')} {api.search()}; <Resultats data={data}/> </> : 
              
              
            {api.setSortBy('prix_croissant'); api.search()}<Resultats data={data}/>}
