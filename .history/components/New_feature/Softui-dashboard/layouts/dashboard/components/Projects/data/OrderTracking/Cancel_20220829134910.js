...
export const Cancel = ({loading, e, CancelOrder, CancelWhyHook, setCancelWhyHook}) => {
  ...
  const compliant = ((CancelWhyHook.canceledWhy != '') && (CancelWhyHook.canceledWhy != "Autre")) || ((CancelWhyHook.canceledWhy = "Autre") && (CancelWhyHook.textAera != ""))
  ...
  <select onChange={(e)=> setCancelWhyHook({
    ...
    className={CancelWhyHook.canceledWhy != '' ? "form-control is-valid" : "form-control is-invalid"} name="choices-button-canceledWhy" id="choices-button-canceledWhy" placeholder="canceledWhy">
  <option value="" className="text-bold" >--Choisir une motif--</option>
  {Reasons.map((r)=> <option value={r} selected={r == CancelWhyHook.canceledWhy ? true : false} >{r}</option>)}

</select>
{CancelWhyHook.canceledWhy === "Autre" ? 
