...
try{
  ...
  const AcceptOrder = async () => {
    ...
    const  DatabaseSeed = {id: e.id, accepted: true, acceptedBy: user?.email, acceptedAt: datetime, pending}
    update(DatabaseSeed)
  }
...
<p class={e.sold ? "col-4 text-end text-bold text-success" : e.canceled ? "col-4 text-end text-bold text-danger" : e.accepted ? "col-4 text-end text-bold text-secondary" : e.pending ? "col-4 text-end text-bold text-secondary" : ""}>{e.canceled ? "Annulée" : "Vendue"}</p>
  ...
  <div class="row justify-content-center">
  <div class="col-6">
  <button type="button" class="btn bg-gradient-danger" data-bs-toggle="modal-order" data-bs-target="#confirmation" id="btn-fontsize-product" data-animation="true" disabled={(e.canceled || e.sold || e.pending) ? true: false}>Annuler la commande</button>
  </div>
  <div class="col-6">
  <button type="button" class="btn bg-gradient-info" data-bs-toggle="modal-order" data-bs-target="#confirmation" id="btn-fontsize-product" onClick={AcceptOrder} data-animation="true" disabled={(e.accepted || e.sold) ? true: false}>Accepter la commande</button>
  </div>
  <div class="col-6">
  <button type="button" class="btn bg-gradient-success" data-bs-toggle="modal-order" data-bs-target="#confirmation" id="btn-fontsize-product" data-animation="true" disabled={(e.sold || e.pending || e.canceled) ? true: false}>Marquer comme vendue</button>
  </div>
  ...
  <div class="modal-dialog modal-dialog-centered modal-sm" role="document">
    <div class="modal-content">
      ...
      <div class="col-6"><button type="button" class="btn bg-gradient-danger" data-bs-dismiss="modal-order">Annuler</button></div>
