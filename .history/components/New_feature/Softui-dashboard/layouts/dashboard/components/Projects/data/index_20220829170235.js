...
const [user, loadingUser, errorUser] = useAuthState(firebase.auth());
  ...
  const showBlur = node.acceptedBy === '' ? false: user ? user?.email === "samiboudehri@hotmail.fr" ? false : 
  (node.pending || (user?.email != (node.acceptedBy || node.canceledBy || node.soldBy))) : true : true
