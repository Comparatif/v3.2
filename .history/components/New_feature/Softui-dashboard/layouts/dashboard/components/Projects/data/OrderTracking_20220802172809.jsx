...
<p class={e.sold ? "col-4 text-end text-bold text-success" : e.canceled ? "col-4 text-end text-bold text-danger" : e.accepted ? "col-4 text-end text-bold text-secondary" : e.pending ? "col-4 text-end text-bold text-secondary" : ""}>{e.canceled ? "Annulée" : "Vendue"}</p>
  ...
  <div class="row justify-content-center">
  <div class="col-6">
  <button type="button" class="btn bg-gradient-danger" id="btn-fontsize-product" data-animation="true" disabled={e.canceled ? true: false}>Annuler la commande</button>
  </div>
  <div class="col-6">
  <button type="button" class="btn bg-gradient-info" id="btn-fontsize-product" onClick={()=> console.log("test")} data-animation="true" disabled={e.accepted ? true: false}>Accepter la commande</button>
  </div>
  <div class="col-6">
  <button type="button" class="btn bg-gradient-success" id="btn-fontsize-product" data-animation="true" disabled={e.sold || e.pending ? true: false}>Marquer comme vendue</button>
  </div>
  </div> 
