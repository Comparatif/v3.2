...
let denyAccess
  ...
  if (!user) {denyAccess = true}
  if (e.accepted) {if (user?.email != e.acceptedBy) {denyAccess = true}};
  if (e.canceled) {if (user?.email != e.canceledBy) {denyAccess = true}};
  if (e.sold) {if (user?.email != e.soldBy) {denyAccess = true}};
  if (e.pending) { denyAccess = false };
  if (user?.email === "samiboudehri@hotmail.fr") { denyAccess = false };
...
<p class={e.sold ? "col-4 text-end text-bold text-success" : e.canceled ? "col-4 text-end text-bold text-danger" : e.accepted ? "col-4 text-end text-bold text-secondary" : e.pending ? "col-4 text-end text-bold text-secondary" : ""}>{e.canceled ? "Annulée" : "Vendue"}</p>
  ...
  <div class="row justify-content-center">
{denyAccess ?  <p>Vous n'avez pas la permission de modifier cette commande</p> :
...
<>
  ...
  <div class="col-6">
  <button type="button" class="btn bg-gradient-success" data-bs-toggle="modal" data-bs-target="#sell" id="btn-fontsize-product" onClick={resetMessage} data-animation="true">Marquer comme vendue</button>
  </div>
  </>
  : null}
