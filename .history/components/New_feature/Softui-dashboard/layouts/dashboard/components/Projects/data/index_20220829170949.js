...
const [user, loadingUser, errorUser] = useAuthState(firebase.auth());
  ...
  let showBlurs
  if (!user) {showBlur = false}
  if (user?.email === "samiboudehri@hotmail.fr") { showBlur = false };
  if (node.pending) { showBlurs = false };
  if (user?.email != node.acceptedBy) {showBlur = false}
  if (user?.email != node.canceledBy) {showBlur = false}
  if (user?.email != node.soldBy) {showBlur = false}
