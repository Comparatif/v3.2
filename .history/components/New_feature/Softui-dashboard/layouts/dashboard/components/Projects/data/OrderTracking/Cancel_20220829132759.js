...
export const Cancel = ({loading, e, CancelOrder, setCancelWhyHook}) => {
  const [data, setData] = useState({
    cancelWhy: '',
    textAera: '',
  })
  const compliant = ((data.cancelWhy != '') && (data.cancelWhy != "Autre")) || ((data.cancelWhy = "Autre") && (data.textAera != ""))
  const CommentToReturn = data.cancelWhy + ": " + data.textAera
...
<div class="form-group">
  ...
  :null
        ...
        <button type="button" disabled={!compliant} class="btn bg-gradient-danger col-5 p-2 m-2" onClick={()=> CancelOrder({CommentToReturn})}>Confirmer</button>
        </>
      }
        </div>
