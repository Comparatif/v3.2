...
const [user, loadingUser, errorUser] = useAuthState(firebase.auth());
  ...
  const showBlur = user ? user?.email === "samiboudehri@hotmail.fr" ? false : 
  node.pending || (user?.email != (node.acceptedBy || node.canceledBy || node.soldBy)) : true 

  let showBlurs
  if (!user) {showBlurs = false}
  if (user?.email === "samiboudehri@hotmail.fr") { showBlurs = false };
  if (node.pending) { showBlurs = false };
  if (user?.email != node.acceptedBy) {showBlurs = false}
  if (user?.email != node.canceledBy) {showBlurs = false}
  if (user?.email != node.soldBy) {showBlurs = false}
