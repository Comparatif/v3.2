...
<p class={e.sold ? "col-4 text-end text-bold text-success" : e.canceled ? "col-4 text-end text-bold text-danger" : e.accepted ? "col-4 text-end text-bold text-secondary" : e.pending ? "col-4 text-end text-bold text-secondary" : ""}>{e.canceled ? "Annulée" : "Vendue"}</p>
  ...
  <div class="col-6">
  <button type="button" class="btn bg-gradient-info" data-bs-toggle="modal" data-bs-target="#confirmation" id="btn-fontsize-product" onClick={AcceptOrder} data-animation="true" disabled={(e.accepted || e.sold) ? true: false}>Accepter la commande</button>
  </div>
  <div class="col-6">
