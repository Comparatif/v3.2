...
export default function data() {
  const { data, error, loading} = useQuery(QueryOrders);

  

  const [Data, setData] = useState()
  const {TrackContext, SetTrackContext} = useContext(TrackingContext)
  const cartItems = useReactiveVar(Sub);
...
/*
  ...
  console.log(Data)*/
    ...
    const filteredData = data?.data?.map((e)=> Object.assign({Boutiques: e.vendeurs, Prix: e.product_price},{}))
    const filterTest = data?.data?.map((e)=> {
