...
<ul class="dropdown-menu z-index-1" aria-labelledby={cle}>
  ...
  <img src="/img/custom/info.svg"/> Détails
    ...
    const NoResults = <tr>{LeftField} <td class="results_ligne">Aucun résultats, réessayez :(</td></tr>
    const CheckRelevance = data?.results?.hits?.sortedBy == "relevance"
    const CheckPrice = data?.results?.hits?.sortedBy == "prix_croissant"
    const RelevanceSearch = <tr>{api.setSortBy('relevance')}  <>{LeftField}  {resultats}</></tr>
    const PriceSearch = <tr>{api.setSortBy('prix_croissant')}  <>{LeftField}  {resultats}</></tr>
    ...
    return(
            ...
            loading ? LoadingLogo : (!CheckIfZeroResults && CheckPrice) ? 
              
            <> {PriceSearch} </> : ( CheckRelevance && CheckIfZeroResults ? <> {NoResults} </> : RelevanceSearch ) 
