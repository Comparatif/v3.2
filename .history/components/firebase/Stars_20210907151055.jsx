...
export const AverageStars = ({boutique_infos, boutique, user, loading, datetime, cle}) => {
  ...
  //console.log(canevas)
  const ShouldCanevasDisapear = canevas?.map((a) => a.id).slice(-1).includes("SignUp-form-" + cle)
  console.log(canevas > 1 ? canevas?.map((a) => a.id).slice(-1).includes("SignUp-form-" + cle) : "")
          })
