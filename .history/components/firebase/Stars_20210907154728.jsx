...
export const AverageStars = ({boutique_infos, boutique, user, loading, datetime, cle}) => {
  ...
  */
    ...
    useEffect(() =>
    {
      ...
      const ContientLaValeur = canevas?.map((a) => a.id).at(-1).toString().includes("SignUp-form-" + cle)
      var ShouldCanevasDisapear = canevas?.length > 1 ? canevas?.map((a) => a.id).at(-1).toString().includes("SignUp-form-" + cle) : "un canevas ouvert uniquement"
      console.log(ShouldCanevasDisapear)
