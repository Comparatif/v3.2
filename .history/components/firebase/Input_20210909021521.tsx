            ...
            <form onSubmit={handleSubmit(onSubmit)} id="contact-form" >
              <div className="card-body pb-2">
                ...
                <div className="row">
                  ...
                  <button type="submit" className="btn bg-gradient-primary mt-3 mb-0" onClick={getValues("commentaire")?.length > 0 ? (() => setSubmit(true)) : (() => setSubmit(false))} {...getValues("commentaire")?.length > 0 ? {disabled:false} : {disabled:true}}>{isSubmited ? "Avis envoyé !" : "Donnez votre avis"}</button>
                  </div>
                </div>
              </div>
            </form>
