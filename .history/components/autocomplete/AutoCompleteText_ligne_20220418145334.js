...
class AutoCompleteText_ligne extends React.Component {
  ...
  render() {
    ...
    const { toggleIsOn } = this.props
    const { setQuery } = this.props
    const node = isBrowser ? document.getElementsByClassName("autocomplete-input-ligne")[0] : ""
    
    node != undefined ? node.addEventListener("keyup", ({key}) => {
      if (key === "Enter") {
        setQuery(text); api.setQuery(text); api.search(); toggleIsOn()
      }
  }) : ""
  ...
  <form class="form-autocomplete-ligne">
    <div class="tb">
      ...
      <div class="td" id="s-cover-ligne">
        ...
        <button type="submit" class="autocomplete-button-ligne" onClick={()=>{setQuery(text); api.setQuery(text); api.search(); toggleIsOn()}} >
