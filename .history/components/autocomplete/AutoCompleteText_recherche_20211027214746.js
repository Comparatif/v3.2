...
class AutoCompleteText_recherche extends React.Component {
  ...
  onTextChanged = (e) => {
    ...
    if (value.length > 0) {
      ...
      this.setState(() => ({suggestions, text: value, url: isBrowser ? window.location.host : '' }));
  }
  ...
  renderSuggestions () {
    ...
    console.log(isBrowser ? window.location.host : '')
    ...
    return (
      ...
      <a 
      href={isBrowser ? (window.location.host + '/' + item.replace(/ /g, '-')) : '' } 
