...
class AutoCompleteText_ligne extends React.Component {
  ...
  renderSuggestions () {
    ...
    const listOfItems = SearchState?.test123.split(",")
    const newlist = (newtext) => listOfItems?.map((item, index) => { return index == id-1 ? newtext : item}).join(",")
    ...
    return (
      ...
      onClick={() => {this.suggestionSelected(item);
                      ...
                      api.search(); 
                      toggleIsOn();
                      updatetest123({id: SearchState?.id, test123: newlist(item.replace(",",""))})
  ...
  render() {
    ...
    node != undefined ? node.addEventListener("keyup", ({key}) => {
      if (key === "Enter") {
        ...
        api.search(); 
        toggleIsOn();
        updatetest123({id: SearchState?.id, test123: newlist(text.replace(",",""))})
      }
  }) : ""
  ...
  <form className="form-autocomplete-ligne">
    <div className="tb">
      <div className="td">
      <input className="autocomplete-input-ligne" value= {text} onChange={this.onTextChanged} type="text" placeholder={api.getQuery() == "" ? "Rechercher..." : api.getQuery()}/>
