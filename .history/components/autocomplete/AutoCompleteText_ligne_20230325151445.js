...
class AutoCompleteText_ligne extends React.Component {
  ...
  renderSuggestions () {
    ...
    const { updatetest123 } = this.props
  ...
  render() {
    ...
    const itemToReplace = SearchState.test123?.split("##!;#2")[id-1]
    const listOfItems = SearchState?.test123
    listOfItems.forEach(function(item, i) { if (item == itemToReplace) a[i] = text; });
    console.log(listOfItems.forEach(function(item, i) { if (item == itemToReplace) a[i] = text; }););
    ...
    node != undefined ? node.addEventListener("keyup", ({key}) => {
      if (key === "Enter") {
        ...
        api.search(); 
        toggleIsOn();
        updatetest123({id: SearchState.id, test123: listOfItems.forEach(function(item, i) { if (item == itemToReplace) a[i] = text; })})
      }
  }) : ""
