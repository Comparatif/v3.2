...
class AutoCompleteText_ligne extends React.Component {
  ...
  render() {
    ...
    console.log(listOfItems.map(function(item) { return item == sss ? "text" : item}))
    console.log(listOfItems.map(function(item) { return item == sss ? "text" : item}).join("##!;#2"))
    ...
    node != undefined ? node.addEventListener("keyup", ({key}) => {
      if (key === "Enter") {
        ...
        api.search(); 
        toggleIsOn();
        updatetest123({id: SearchState.id, test123: "AMD Ryzen 9 5900X##!;#2Geforce RTX 2060##!;#2AMD B550"})
      }
  }) : ""
