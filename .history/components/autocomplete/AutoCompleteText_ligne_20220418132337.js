...
class AutoCompleteText_ligne extends React.Component {
  ...
  renderSuggestions () {
    ...
    return (
      ...
      <div id="suggestions_recherche_ligne" class="bg-gradient-info opacity-8">
      ...
      onClick={() => {this.suggestionSelected(item);setQuery(item); api.setQuery(item); api.setSortBy('prix_croissant'); api.search(); toggleIsOn()}}>
  ...
  render() {
    ...
    node != undefined ? node.addEventListener("keyup", ({key}) => {
      if (key === "Enter") {
        setQuery(text); api.setQuery(text); api.setSortBy('prix_croissant');api.search(); toggleIsOn()
      }
  }) : ""
    ...
    return (
      ...
      <div class="container bg-gradient-info opacity-8" id="cover-autocomplete-ligne">
      
  <form class="form-autocomplete-ligne">
    <div class="tb">
      <div class="td">
      ...
      <div class="td" id="s-cover-ligne">
        ...
        <button type="submit" class="autocomplete-button-ligne" onClick={()=>{setQuery(text); api.setQuery(text); api.setSortBy('prix_croissant');api.search(); toggleIsOn()}} >
