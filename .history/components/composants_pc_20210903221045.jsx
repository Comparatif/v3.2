...
const part1  = `
  query resultSet($query: String, $filters: [SKFiltersSet], $page: SKPageInput, $sortBy: String) {
    results(query: $query, filters: $filters, queryOptions: $queryOptions) {
      summary {
        total
        appliedFilters {
          id
...
const query1 = gql`${part1 }`
const query2 = gql`${part1 + `"or"` }`
...
const Page = () => {
  ...
  const query = data?.results?.hits.sortedBy == "prix_croissant" ? query1 : query2

  const variables = useSearchkitVariables()
  const { previousData, data = previousData, loading } = useQuery(part1, { variables })
  ...
  <header class="bg-gradient-dark position-fixed w-100 z-index-2">
    <div class="page-header" id="page-header-custom-recherche">
      ...
      <div class="position-absolute w-100 z-index-1 bottom-0">
        ...
        <svg class="waves" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 24 150 40" preserveAspectRatio="none" shape-rendering="auto">
          <defs>
