            ...
            args: {
             ...
             async resolve(_, args, ctx) {
                ...
                if (queryResults.length > 0) {
                    ...
                    // first cursor we'll return in subsequent requests
                    const firstCursor = firstOrderInResults.id;
                    ...
                    const secondQueryResultsPrevious = await ctx.prisma.order.findMany({
                        take: args.last,
                        cursor: {
                            id: firstCursor,
                        },
                    ...
                    const result = {
                        pageInfo: {
                            endCursor: endCursor,
                            hasNextPage: secondQueryResultsNe.length >= args.first, // if the number of items requested is greater than the response of the second query, we have another page
